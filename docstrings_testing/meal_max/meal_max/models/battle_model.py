...
class BattleModel:
    """
    A class that keeps track of the of battle between the combatants


    Attributes:
        combatants(List[Meal]) is the list of meals for the combatants
    ...
    def battle(self) -> str:
        """
        Updates the stats of both the combatant, and determines the winner's meal


        Raises:
            ValueError: If there is not enough combatants.
    ...
    def get_battle_score(self, combatant: Meal) -> float:
        """
        Calculates a battle score for a given Meal object based on its price, cuisine type, and difficulty level.


        Arg:
            combatant (Meal): The Meal object for which the battle score is being calculated.
    ...
    def prep_combatant(self, combatant_data: Meal):
        """
        Adds the combatant to the list


        Arg:
            combatant_data (Meal): The meal object of the combatant we want to add to the list of combatants.
